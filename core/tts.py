...
class KokoroTTSEngine:
    ...
    def _init(self) -> None:
        ...
        try:
            ...
            if any(k in _e for k in ("offline", "not found", "cache", "localentry", "does not exist", "unable to open file", "model.bin")):
                print("[TTS] Kokoro model cache is incomplete or offline — refreshing the model download…")
                reset_hf_offline_flags()
                removed = remove_broken_hf_snapshot("kokoro")
                if removed:
                    print(f"[TTS] Removed broken cache snapshot(s): {', '.join(removed)}")
                self._pipeline = _create_pipeline()
            else:
                raise
...
# ---------------------------------------------------------------------------
# Language-to-voice mappings for auto-switching
# ---------------------------------------------------------------------------

LANG_TO_EDGETTS_VOICE = {
...
class TTSPlayer:
    ...
    Also supports dynamic language switching.
    """
    ...
    def set_language(self, lang_code: str) -> bool:
        """
        Switch TTS voice to match a language (ISO 639-1 code).
        Returns True if the voice was changed, False if unsupported.
        """
        ...
        if engine_name == "kokoro":
            voice = self._kokoro_lang_to_voice(lang_code)
            if not voice:
                return False
            try:
                speed = float(self._config.get("tts_speed", 1.0))
                new_engine = KokoroTTSEngine(voice=voice, speed=speed)
                self._engine = new_engine
                self._config["tts_voice"] = voice
                print(f"[TTS] Language switched to '{lang_code}' (Kokoro voice: {voice})")
                return True
            except Exception as e:
                print(f"[TTS] Language switch failed: {e}")
                return False

        voice = LANG_TO_EDGETTS_VOICE.get(lang_code[:2])
        if not voice:
            return False
        try:
            new_engine = EdgeTTSEngine(voice=voice)
            self._engine = new_engine
            self._config["tts_voice"] = voice
            print(f"[TTS] Language switched to '{lang_code}' (EdgeTTS voice: {voice})")
            return True
        except Exception as e:
            print(f"[TTS] Language switch failed: {e}")
            return False
    ...
    @staticmethod
    def _kokoro_lang_to_voice(lang_code: str) -> str | None:
        return LANG_TO_KOKORO_VOICE.get(lang_code[:2])
...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    return TTSPlayer(engine, config=config)
