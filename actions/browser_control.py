...
def _normalize_url(url: str) -> str:
    ...
    # Detect Wikipedia queries
    _wiki = re.search(
        r'(?:open\s+|go\s+to\s+|search\s+)?(?:wikipedia\s+)?(.+?)(?:\s+(?:page|article|on|in)\s+|\s+from\s+)?\s*(?:wikipedia|wiki)',
        url, re.IGNORECASE,
    )
    if _wiki:
        topic = _wiki.group(1).strip().rstrip(".,!?")
        if topic and topic.lower() not in ("wikipedia", "wiki", ""):
            topic_clean = topic.replace(" page", "").replace(" article", "").strip()
            return f"https://en.wikipedia.org/wiki/{_url_quote(topic_clean.replace(' ', '_'))}"

    # Detect "page on X" patterns for known sites
    known_sites = {
        "wikipedia": "en.wikipedia.org/wiki/",
    }
    for site, path in known_sites.items():
        pattern = re.search(rf'(.+?)\s+(?:page|article)\s+(?:on|from|in)\s+{site}', url, re.IGNORECASE)
        if pattern:
            topic = pattern.group(1).strip().rstrip(".,!?")
            return f"https://{path}{_url_quote(topic.replace(' ', '_'))}"

    # Detect "X in/on github" patterns (handles both "X in github" and "X repo in/on github")
    github_match = re.search(r'(.+?)\s+(?:repo\s+)?(?:in|on)\s+github', url, re.IGNORECASE)
    if github_match:
        query = github_match.group(1).strip().rstrip(".,!?")
        parts = [p for p in query.split() if p.lower() not in ("open", "go", "to", "the", "a")]
        if len(parts) == 1:
    ...
    # Bare word or phrase → Google search
...
def browser_control(
    ...
    if not _PLAYWRIGHT_AVAILABLE:
        return "Browser control requires playwright — install it with: pip install playwright && playwright install chromium"
    params  = parameters or {}
    action  = params.get("action", "").lower().strip()
    browser = params.get("browser", "").lower().strip() or None
    result  = "Unknown action."

    # go_to and search are simple navigation — open in default browser, skip Playwright
    if action == "go_to":
        url = _normalize_url(params.get("url", ""))
        if _open_native(url):
            _log(player, f"Opened {url} in default browser")
            return f"Opened {url} in your browser."
        # fallback to Playwright if native fails
        _log(player, "Native open failed, trying Playwright...")
    ...
    if action == "search":
        ...
        if _open_native(url):
            ...
            return f"Searched {engine} for '{query}' in your browser."
        # fallback to Playwright
        _log(player, "Native search failed, trying Playwright...")
